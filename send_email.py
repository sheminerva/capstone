...
def extract_summary(file_path="data_input/data.csv", id=['936','1178']):
  ...
  start_date = campaigns['reporting_start'].min().strftime("%d %b %Y")
  end_date = campaigns['reporting_end'].max().strftime("%d %b %Y")
...
def compose_email(template, name, data_dict):
  ...
  composed = template.substitute(
    ...
    GITHUB_LINK='https://github.com/sheminerva/capstone'
  )
  return composed
